...
import cv2
import os
import csv      # it stands for "comma" "seperated" "values"
from tkinter import filedialog



mydata=[] #took my data as list
class Attendance:
    def __init__(self,root):
        ...
        img=img.resize((1530,200),Image.ANTIALIAS)#antialias high quality image ko low qualiity image pe set krta h
        ...
        #img1=img1.resize((800,200),Image.ANTIALIAS)#antialias high quality image ko low qualiity image pe set krta h
        #self.photoimg1=ImageTk.PhotoImage(img1)

        #f_lbl=Label(self.root,image=self.photoimg1)
       # f_lbl.place(x=800,y=0,width=550,height=200)
        ...
        TITLE_LBL=Label(bg_img,text="ATTENDANCE MANAGEMENT SYSTEM",font=("times new roman",40,"bold"),bg="darkblue",fg="red")
        ...
        left_inside_frame.place(x=0,y=135,width=720,height=370)
            ...
            #Labels entry
            #Attendance ID
        attendanceid_label=Label( left_inside_frame,text="AttendanceID : ",font=("comicsansns",13,"bold"),background="white")
        attendanceid_label.grid(row=0,column=0,padx=10,pady=5,sticky=W)


        attendanceid_entry=ttk.Entry(left_inside_frame,textvariable=self.var_atten_id,width=20,font=("comicsansns",13,"bold"))
        ...
        atten_dep=ttk.Entry(left_inside_frame,textvariable=self.var_atten_dept, width=22, font="comicsansns 11 bold")
        ...
        #===================== SCROLL BAR =======================
        scroll_x=ttk.Scrollbar(table_frame,orient=HORIZONTAL) #with the help of ttk we can make scrollbar
        scroll_y=ttk.Scrollbar(table_frame,orient=VERTICAL)


        self.AttendanceReportTable=ttk.Treeview(table_frame,column=("id","roll","name","department","time","date","attendance"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
        #treeview is used to make header name in column
...
#====================Fetch Data======================
    def fetchData(self,rows):
        self.AttendanceReportTable.delete(self.AttendanceReportTable.get_children()) #get_children() method return a list of the child Artists of this Artist
        for i in rows :
...
#By default, the print function ends with a newline.
        ...
        #==========IMPORT===========
    def importCsv(self):        
        global mydata #global variable in Python means having a scope throughout the program, i.e., a global variable value is accessible throughout the program unless shadowed
        mydata.clear()#this helps in adding data after clearing the data of previous file 
        fln=filedialog.askopenfilename(initialdir=os.getcwd(),title="Open CSV",filetypes=(("CSV File","*.csv"),("All file","*.*")),parent=self.root)#getcwd mean get current working directory
        #filedialog module provides classes and factory functions for creating file/directory selection windows
        with open(fln) as myfile:
            csvread=csv.reader(myfile,delimiter=",")#delimiter helps in defining the seperator
            for i in csvread:
                mydata.append(i)
    ...
    def exportCsv(self):
        try:
            ...
            with open(fln,mode="w",newline="") as myfile:
                ...
                messagebox.showinfo("DATA EXPORT","You're Data Exported !!!!")
        except Exception as es: #the except Exception as es statement is a statement that defines an argument to the except statement
